...
try:
    from bot.agent import SQLResponse, DataGovResponse, CypherQuery
...
settings = Settings(path=MCP_DIR / "settings.yaml")
...
def cypher_query(query: CypherQuery, ctx: Context) -> DataGovResponse:
    """业务域、应用、数据实体、物理表信息查询工具。需要使用'cypher_query_prompt'提示词生成合适的Cypher语句。
...
引用@schema://data_governance 获得数据治理图数据结构
        """),
